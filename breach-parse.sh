...
if [ $# -lt 2 ]; then
    ...
    echo 'Example: ./breach-parse.sh @gmail.com gmail.txt "~/Downloads/BreachCompilation"'
    echo "You only need to specify [breach data location] if its not in the expected location (${breachDataLocation})"
    echo " "
    ...
    if [ $# -eq 3 ]; then
        if [ -d "$3" ]; then
            ...
            echo 'Pass the BreachCompilation directory as the third argument'
            echo 'Example: ./breach-parse.sh @gmail.com gmail.txt "~/Downloads/BreachCompilation"'
            exit 1
        fi
    else
        if [ ! -d "${breachDataLocation}" ]; then
            ...
            echo 'Put the breached password list there or specify the location of the BreachCompilation as the third argument'
            echo 'Example: ./breach-parse.sh @gmail.com gmail.txt "~/Downloads/BreachCompilation"'
            exit 1
        fi
    fi
    ...
    total_Files=$(find "$breachDataLocation/data" -type f -not -path '*/\.*' | wc -l)
    file_Count=0
    ...
    # grep for passwords
    find "$breachDataLocation/data" -type f -not -path '*/\.*' -print0 | while read -d $'\0' file; do
